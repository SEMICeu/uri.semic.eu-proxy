...
server {
    ...
    set $match $1;
    proxy_pass   http://matomo:80$match$is_args$args;
    }
    ...
    proxy_set_header X-Real-IP $remote_addr;
    proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
    proxy_set_header X-Forwarded-Uri /virtuoso;

    set $match $1;
    proxy_pass   http://exp-virtuoso:8890/$match$is_args$args;
    }
    ...
    set $match $1;
    proxy_pass   http://exp-virtuoso:8890/conductor$match$is_args$args;
    }
...
-- if the uri has trailing slash
	 ...
	 local machineprocessable = true 
		...
		derived_extension = reqext
             ...
             uri = ngx.re.sub(entity_noextension, "^/2qy/(.*)", "/uri.semic.eu-puris/releases/2qy/$1." .. derived_extension, "o")
         else
            -- check if there is a machine readible file extension

	     if entity_extension ~= nil and urisemic.supportedext(entity_extension) then
                uri = ngx.re.sub(entity_noextension, "^/2qy/(.*)", "/uri.semic.eu-puris/releases/2qy/$1." .. entity_extension, "o")
             else 
		     ...
		     if htmlmap.htmlmap[entity_noextension] then
			    ...
			    -- point to a default html page on the r5r vocabulary
--			    uri = ngx.re.sub(entity_noextension, "^/2qy/(.*)", "https://semiceu.github.io/RRMV/releases/0.1.1/", "o")
			    uri = "https://semiceu.github.io/RRMV/releases/0.2.0/"
		    end
              end
         end
...
-- if the uri has trailing slash
	 ...
	 local machineprocessable = true 
		...
		derived_extension = reqext
             ...
             uri = ngx.re.sub(entity_noextension, "^/s1n/(.*)", "/uri.semic.eu-puris/releases/s1n/$1." .. derived_extension, "o")
         else
            -- check if there is a machine readible file extension

	     if entity_extension ~= nil and urisemic.supportedext(entity_extension) then
                uri = ngx.re.sub(entity_noextension, "^/s1n/(.*)", "/uri.semic.eu-puris/releases/s1n/$1." .. entity_extension, "o")
             else 
		     ...
		     if htmlmap.htmlmap[entity_noextension] then
			    ...
			    -- point to a default html page on the core vocs
--			    uri = ngx.re.sub(entity_noextension, "^/s1n/(.*)", "https://joinup.ec.europa.eu/solution/e-government-core-vocabularies", "o")
--			    uri = "https://joinup.ec.europa.eu/solution/e-government-core-vocabularies"
			    uri = "https://SEMICeu.github.io/uri.semic.eu-puris/releases/s1n/s1n.ttl"
		    end
              end
         end
